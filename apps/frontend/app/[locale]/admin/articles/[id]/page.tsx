...
export default function EditArticlePage({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("saveFailedEdit"));
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!article)
    return (
      <p style={{ color: "#ef4444", padding: "40px" }}>{t("articleNotFound")}</p>
    );
    ...
    <div className={styles.container}>
      <div className={styles.headerRow}>
        <div>
          ...
          <h2 className={styles.title}>{t("editPageTitle")}</h2>
        </div>
        ...
        <aside className={styles.sidebar}>
          <Card className={styles.sideCard}>
            ...
            <div className={styles.toggleRow}>
              <span className={styles.sideLabel}>{t("published")}</span>
              <button
