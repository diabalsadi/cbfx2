...
export async function generateMetadata({
  ...
  const seo = await getSeoMeta("cashback", locale);
...
export default async function Page({
  ...
  return (
    <>
      <JsonLd data={webPageJsonLd({ name: seo.title, description: seo.description, path: `/${locale}${seo.canonical_path ?? "/cashback"}` })} />
      <CashbackClient />
    </>
  );
}
