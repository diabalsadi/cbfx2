...
const navItems = [
  { label: 'Overview',  href: '/admin/overview',     icon: '◈', roles: ['super_admin', 'broker'] },
  { label: 'Users',     href: '/admin/users',         icon: '⊹', roles: ['super_admin'] },
  { label: 'Articles',  href: '/admin/articles',      icon: '◎', roles: ['super_admin', 'editor'] },
  { label: 'Campaigns', href: '/admin/ads-campaigns', icon: '◉', roles: ['super_admin', 'broker'] },
  { label: 'Clients',   href: '/admin/clients',       icon: '◫', roles: ['super_admin', 'broker'] },
  { label: 'Reports',   href: '/admin/reports',       icon: '▣', roles: ['super_admin', 'broker'] },
];
...
const Layout = ({ children, className }: ILayoutProps) => {
  ...
  return (
    <div className={style.container}>
      {isSidebarOpen && <div className={style.overlay} onClick={toggleSidebar} />}
      ...
      <aside className={cx(style.sidebar, { [style.open]: isSidebarOpen })}>
        <div className={style.sidebarInner}>
          <div className={style.sidebarLogo}>
            ...
            <button className={style.closeTrigger} onClick={toggleSidebar} aria-label="Close">✕</button>
          </div>
              ...
              {filteredNav.map(item => {
                ...
                return (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      className={cx(style.navItem, { [style.active]: isActive })}
          ...
          <div className={style.sidebarUser}>
            ...
            <button className={style.logoutBtn} onClick={logout} title="Sign out">→</button>
          </div>
        </div>
      </aside>
      ...
      <div className={style.main}>
        <header className={style.topbar}>
          <div className={style.topbarLeft}>
            <button className={style.menuBtn} onClick={toggleSidebar} aria-label="Menu">
              <span /><span /><span />
            </button>
            <h1 className={style.pageTitle}>{getTitle()}</h1>
          </div>
          <div className={style.topbarRight}>
            <button className={style.themeToggle} onClick={toggleTheme} aria-label="Toggle theme">
              {theme === 'dark' ? '☀︎' : '☽'}
            </button>
          </div>
        </header>
