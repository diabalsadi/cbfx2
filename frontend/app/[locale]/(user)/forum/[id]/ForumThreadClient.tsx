...
const CATEGORIES = ["All", "General", "Forex", "Crypto", "Metals", "Indices", "Strategy"] as const;
...
export default function ThreadPage({
  ...
  async function submitReply(event: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("unableToPost"));
    } finally {
      setSaving(false);
    }
  }
  ...
  if (!thread) return <p className={styles.empty}>{t("loadingDiscussion")}</p>;
    ...
    <div className={styles.page}>
      ...
      <article className={styles.detail}>
        <div className={styles.threadTop}>
          ...
          {thread.is_pinned && <span className={styles.pinned}>{t("pinned")}</span>}
        </div>
        <h1>{thread.title}</h1>
        ...
        <div className={styles.meta}>
          {t("startedBy", { author: thread.author_email.split("@")[0], count: thread.replies.length })}
        </div>
      </article>
      ...
      <section className={styles.replies}>
        ...
        {thread.replies.length === 0 && (
          <p className={styles.empty}>{t("beFirstToReply")}</p>
        )}
      </section>

      <form className={styles.replyForm} onSubmit={submitReply}>
        <label>{t("joinDiscussion")}</label>
        <textarea
          value={reply}
          onChange={(e) => setReply(e.target.value)}
          placeholder={user ? t("replyPlaceholderIn") : t("replyPlaceholderOut")}
          disabled={!user}
          rows={4}
        />
