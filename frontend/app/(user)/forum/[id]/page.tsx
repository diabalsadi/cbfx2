...
export default function ThreadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function submitReply(event: FormEvent) {
    ...
    try {
      ...
      setThread(current => current ? { ...current, reply_count: current.reply_count + 1, replies: [...current.replies, created] } : current);
      setReply('');
      setImageFile(null);
      setImagePreview(null);
      if (fileInputRef.current) fileInputRef.current.value = '';
  ...
  return (
    <div className={styles.page}>
      <Link href="/forum" className={styles.back}>← Back to forum</Link>
      <article className={styles.detail}>
        <div className={styles.threadTop}><span className={styles.category}>{thread.category}</span>{thread.is_pinned && <span className={styles.pinned}>Pinned</span>}</div>
        <h1>{thread.title}</h1>
        {thread.body && <p>{thread.body}</p>}
        {thread.image_url && <img src={thread.image_url} alt={thread.title} className={styles.threadImage} />}
        <div className={styles.meta}>Started by {thread.author_email.split('@')[0]} · {thread.replies.length} replies</div>
      </article>

      <section className={styles.replies}>
        <h2>Replies</h2>
        {thread.replies.map(item => <div key={item.id} className={styles.reply}><div className={styles.replyAuthor}>{item.author_email.split('@')[0]}</div>{item.body && <p>{item.body}</p>}{item.image_url && <img src={item.image_url} alt="Reply attachment" className={styles.threadImage} />}</div>)}
        {thread.replies.length === 0 && <p className={styles.empty}>Be the first to reply.</p>}
      </section>
