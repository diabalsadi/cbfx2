...
const MARKETS: Record<Tab, { symbol: string; name: string; price: string; change: string; up: boolean }[]> = {
  Forex: [
  ...
  return (
    <>
      ...
      <div className={styles.list}>
        {filtered.length === 0 ? (
          <div className={styles.empty}>No symbols match &ldquo;{search}&rdquo;</div>
        ) : (
          filtered.map(m => (
            <div key={m.name} className={styles.row} onClick={() => setSelected(m)}>
              ...
              <div className={styles.rowRight}>
                ...
                <div className={`${styles.symbolChange} ${m.up ? styles.up : styles.down}`}>
                  {m.up ? '↗' : '↘'} {m.change}
                </div>
              </div>
            </div>
