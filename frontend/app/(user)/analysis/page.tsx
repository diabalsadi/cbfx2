...
export default function AnalysisPage() {
  ...
  useEffect(() => {
    articlesApi.listAnalysis()
      .then(setArticles)
      .catch((err: unknown) => setError(err instanceof Error ? err.message : 'Unable to load analysis.'))
  ...
  return (
    <>
      <div className={styles.pageHeader}>
        ...
        <div className={styles.filters} aria-label="Filter analysis by market category">
          {CATEGORIES.map(item => (
            <button
              key={item.value}
              type="button"
      ...
      {loading ? (
        ...
        <p className={styles.empty}>No {category} analysis has been published yet.</p>
      ) : (
        <>
          {/* Featured / breaking */}
          {featured && (
          ...
          <div className={styles.latestSection}>
            ...
            <div className={styles.list}>
              {latest.map((a, i) => (
                <Link key={a.id} href={`/analysis/${a.id}`} className={styles.row}>
                  <div className={styles.rowLeft}>
                    {a.symbol && <span className={styles.symbol}>{a.symbol}</span>}
                    <span className={styles.rowTime}>· {timeAgo(a.created_at)}</span>
                  </div>
                  <div className={styles.rowTitle}>{a.title}</div>
                  {i < 2 && <span className={styles.hotBadge}>📈 TRENDING</span>}
                </Link>
              ))}
            </div>
          </div>
