...
export default function AdsCampaignsPage() {
  ...
  const [formData, setFormData] = useState<FormData>({ name: '', budget: '', status: 'draft' });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.post('/campaigns/', { name: formData.name, budget: parseFloat(formData.budget) || 0, status: formData.status });
      setShowForm(false);
    ...
    <div className={styles.container}>
      <div className={styles.headerRow}>
        <div>
          ...
          <p className={styles.subtitle}>Manage advertising campaigns and track performance.</p>
        </div>
          ...
          <form onSubmit={handleCreate} className={styles.form}>
            <div className={styles.formRow}>
              <div className={styles.field}>
                ...
                <input className={styles.input} placeholder="e.g. Summer Push 2025" value={formData.name}
                  onChange={e => setFormData(v => ({ ...v, name: e.target.value }))} required />
              </div>
              <div className={styles.field}>
                <label className={styles.label}>Budget ($)</label>
                <input className={styles.input} type="number" placeholder="5000" value={formData.budget}
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className={styles.empty}>No campaigns yet. Create your first one.</td></tr>
              ) : campaigns.map(c => (
                <tr key={c.id}>
